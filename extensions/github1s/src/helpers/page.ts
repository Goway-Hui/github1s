...
export const createPageHtml = (title: string, styles: string[] = [], scripts: string[] = []) => {
	...
	return `
		...
		<head>
			...
			<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'nonce-${nonce}' 'self' http: https: data:; script-src 'nonce-${nonce}' 'self' http: https: data:; img-src *;">
			<title>${title}</title>
